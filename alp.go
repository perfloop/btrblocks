...
// alpChildBuilder compresses ALP's signed encoded values and each supported
// patch-index width.
...
func encodeALP32(arr array.ArrayCore[float32], children alpChildBuilder) (EncodedArray[float32], error) {
	if arr.Length() == 0 {
	...
	return buildALPArrayTyped(arr, alpFuncs32, children.BuildInt32, children)
}

// encodeALP64 transforms arr and delegates its encoded values and patch indices.
func encodeALP64(arr array.ArrayCore[float64], children alpChildBuilder) (EncodedArray[float64], error) {
	if arr.Length() == 0 {
		return nil, errDataEmpty
	}
	if children == nil {
	...
	return buildALPArrayTyped(arr, alpFuncs64, children.BuildInt64, children)
}

func buildALPArrayTyped[T Float, I SignedInteger](arr array.ArrayCore[T], funcs alpFuncs[T, I], buildEncoded childBuilder[I], children alpChildBuilder) (EncodedArray[T], error) {
	n := arr.Length()
	...
	case maxIdx <= uint64(^uint8(0)):
		return buildALPWithPatches[T, I, uint8](n, e, f, child, funcs, patchIdx, patchVals, children.BuildUint8)
	case maxIdx <= uint64(^uint16(0)):
		return buildALPWithPatches[T, I, uint16](n, e, f, child, funcs, patchIdx, patchVals, children.BuildUint16)
	case maxIdx <= uint64(^uint32(0)):
		return buildALPWithPatches[T, I, uint32](n, e, f, child, funcs, patchIdx, patchVals, children.BuildUint32)
	default:
		return buildALPWithPatches[T, I, uint64](n, e, f, child, funcs, patchIdx, patchVals, children.BuildUint64)
	}
}

func buildALPWithPatches[T Float, I SignedInteger, J UnsignedInteger](n uint64, e, f uint8, child EncodedArray[I], funcs alpFuncs[T, I], patchIdx []uint64, patchVals []T, buildIndices childBuilder[J]) (EncodedArray[T], error) {
	patches, err := buildALPPatchesTyped[T, J](n, patchIdx, patchVals, buildIndices, funcs.toBits)
	if err != nil {
		return nil, err
	}
...
func buildALPPatchesTyped[T Float, J UnsignedInteger](length uint64, patchIdx []uint64, patchVals []T, buildIndices childBuilder[J], toBits func(T) uint64) (*patches[T, J], error) {
