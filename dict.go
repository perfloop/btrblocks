...
func buildIntegerDictFromDistinctWithChildren[T Integer](arr array.ArrayCore[T], distinct distinctLookup[T], children dictionaryChildBuilder[T]) (EncodedArray[T], error) {
	if children == nil {
	...
	return buildDictWithKey(arr, distinct, identityKey[T], valuesCodec, children)
}
...
func buildDictWithKey[T Integer | Float | String, K comparable](arr array.ArrayCore[T], distinct distinctLookup[K], key func(T) K, valuesCodec EncodedArray[T], children dictionaryChildBuilder[T]) (EncodedArray[T], error) {
	switch numDistinct := distinct.Len(); {
	case numDistinct <= 1<<8:
		return buildDictCodes[T, K, uint8](arr, distinct, key, valuesCodec, children.BuildUint8)
	case numDistinct <= 1<<16:
		return buildDictCodes[T, K, uint16](arr, distinct, key, valuesCodec, children.BuildUint16)
	default:
		return buildDictCodes[T, K, uint32](arr, distinct, key, valuesCodec, children.BuildUint32)
	}
}

// buildDictCodes builds the codes array as []I directly from the distinct map.
func buildDictCodes[T Integer | Float | String, K comparable, I UnsignedInteger](arr array.ArrayCore[T], distinct distinctLookup[K], key func(T) K, valuesCodec EncodedArray[T], buildCodes childBuilder[I]) (EncodedArray[T], error) {
	n := arr.Length()
	codes := make([]I, n)
	for i := range n {
...
func buildFloatDictFromDistinctCapacityWithChildren[T Float](arr array.ArrayCore[T], distinct distinctLookup[uint64], expectedDistinct uint64, fromBits func(uint64) T, children dictionaryChildBuilder[T]) (EncodedArray[T], error) {
	if children == nil {
		return nil, ErrBuilderRequired
	}
	...
	return buildDictWithKey(arr, distinct, array.FloatBits[T], valuesCodec, children)
}

// encodeIntegerDict builds an integer dictionary while leaving
// value and ordinal child selection to the caller.
func encodeIntegerDict[T Integer](arr array.ArrayCore[T], distinct map[T]uint64, children dictionaryChildBuilder[T]) (EncodedArray[T], error) {
	return buildIntegerDictFromDistinctWithChildren(arr, mapLookup[T](distinct), children)
}
...
func encodeFloat32Dict(arr array.ArrayCore[float32], distinct map[uint64]uint64, expectedDistinct uint64, children dictionaryChildBuilder[float32]) (EncodedArray[float32], error) {
...
func encodeFloat64Dict(arr array.ArrayCore[float64], distinct map[uint64]uint64, expectedDistinct uint64, children dictionaryChildBuilder[float64]) (EncodedArray[float64], error) {
	return buildFloatDictFromDistinctCapacityWithChildren(arr, mapLookup[uint64](distinct), expectedDistinct, math.Float64frombits, children)
}

func buildStringDictWithChildren[T String](arr array.ArrayCore[T], children dictionaryChildBuilder[T]) (EncodedArray[T], error) {
	if children == nil {
		return nil, ErrBuilderRequired
	}
	n := arr.Length()
	...
	return buildDictWithKey(arr, dict, identityKey[T], valuesCodec, children)
}

// encodeStringDict builds a string dictionary while leaving value
// and ordinal child selection to the caller.
func encodeStringDict[T String](arr array.ArrayCore[T], children dictionaryChildBuilder[T]) (EncodedArray[T], error) {
	return buildStringDictWithChildren(arr, children)
}
