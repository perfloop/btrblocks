...
func TestZigzagCodecInt64RoundTripAndWriteTo(t *testing.T) {
	...
	codec, err := NewZigzagCodec(array.NewPrimitivesUnsafe(data), 1, 0)
...
func TestNewZigzagCodecChoosesSmallestUnsignedChildWidth(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			codec, err := NewZigzagCodec(array.NewPrimitivesUnsafe(tt.data), 1, 0)
			require.NoError(t, err)
...
func TestReadZigzagCodecZeroLengthRoundTrip(t *testing.T) {
	codec, err := NewZigzagCodec(array.NewPrimitivesUnsafe([]int64{}), 1, 0)
	require.NoError(t, err)
...
func TestReadZigzagCodecRejectsMismatchedOuterMetadata(t *testing.T) {
	codec, err := NewZigzagCodec(array.NewPrimitivesUnsafe([]int64{-1, 0, 1}), 1, 0)
	require.NoError(t, err)
