		...
		values: [maxSchemeCount]scheme[T, intStats[T]]{
			...
			},
			{
				...
				estimate: func(stats intStats[T], ctx planContext) schemeEstimate {
					...
					return estimateKnownSequence[T](stats.Source(), stats.isSequence, stats.sequenceBase, stats.sequenceStep)
				},
			},
			{
				kind:  CodecTypeFor,
			...
			},
			{
				...
				estimate: func(stats intStats[T], ctx planContext) schemeEstimate {
					return estimateDelta[T](stats, ctx, family)
				},
			},
			{
				kind: CodecTypeDict,
				...
				estimate: func(stats intStats[T], ctx planContext) schemeEstimate {
					if stats.distinct == nil {
						return estimateIntegerDictBySample[T](stats, ctx)
					}
					return estimateIntegerDict[T, intStats[T]](stats, ctx, stats.distinctCount, stats.avgRunLength)
				},
			},
			{
				kind: CodecTypeRunEnd,
				build: func(arr array.ArrayCore[T], ctx planContext) (EncodedArray[T], error) {
					return buildPrimitiveRunEnd(arr, ctx, array.CmpIntegers[T], compressValues)
				},
				estimate: func(stats intStats[T], ctx planContext) schemeEstimate {
					return estimateRunEnd[T, intStats[T]](stats, ctx, stats.avgRunLength, primitiveCanSample(stats.Source(), ctx))
				},
			},
			{
...
func signedIntegerSchemes[T array.SignedInteger]() schemeSet[T, intStats[T]] {
	set := integerSchemes[T](compressSignedCore[T], signedInteger, resolveSignedDelta[T])
	set.values[set.count] = scheme[T, intStats[T]]{
		kind:  CodecTypeZigZag,
		build: buildZigZag[T],
		estimate: func(stats intStats[T], ctx planContext) schemeEstimate {
			return estimateZigZag[T, intStats[T]](stats, ctx, stats.hasNegative)
		},
	}
	set.count++
	return set
