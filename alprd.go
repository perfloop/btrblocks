...
func encodeALPRD32(arr array.ArrayCore[float32], children alprdChildBuilder) (EncodedArray[float32], error) {
	if arr.Length() == 0 {
	...
	return buildALPRDArrayTyped(arr, alprdFuncs32, children)
}

// encodeALPRD64 transforms arr and leaves patch-index compression to the caller.
func encodeALPRD64(arr array.ArrayCore[float64], children alprdChildBuilder) (EncodedArray[float64], error) {
	if arr.Length() == 0 {
		return nil, errDataEmpty
	}
	if children == nil {
	...
	return buildALPRDArrayTyped(arr, alprdFuncs64, children)
}

func buildALPRDArrayTyped[T Float](arr array.ArrayCore[T], funcs alprdFuncs[T], children alprdChildBuilder) (EncodedArray[T], error) {
	n := arr.Length()
	...
	case maxIdx <= uint64(^uint8(0)):
		return buildALPRDWithPatches[T, uint8](n, dict, leftBuf, rightBuf, funcs, patchIdx, patchVals, children.BuildUint8)
	case maxIdx <= uint64(^uint16(0)):
		return buildALPRDWithPatches[T, uint16](n, dict, leftBuf, rightBuf, funcs, patchIdx, patchVals, children.BuildUint16)
	case maxIdx <= uint64(^uint32(0)):
		return buildALPRDWithPatches[T, uint32](n, dict, leftBuf, rightBuf, funcs, patchIdx, patchVals, children.BuildUint32)
	default:
		return buildALPRDWithPatches[T, uint64](n, dict, leftBuf, rightBuf, funcs, patchIdx, patchVals, children.BuildUint64)
	}
}

func buildALPRDWithPatches[T Float, J UnsignedInteger](n uint64, dict alprdDict, leftBuf, rightBuf []byte, funcs alprdFuncs[T], patchIdx []uint64, patchVals []uint16, buildIndices childBuilder[J]) (EncodedArray[T], error) {
	patches, err := buildALPRDPatchesTyped[J](n, patchIdx, patchVals, buildIndices)
	if err != nil {
		return nil, err
	}
...
func buildALPRDPatchesTyped[J UnsignedInteger](length uint64, patchIdx []uint64, patchVals []uint16, buildIndices childBuilder[J]) (*patches[uint16, J], error) {
