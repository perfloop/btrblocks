...
func buildZigZagTest[T SignedInteger](arr array.ArrayCore[T]) (EncodedArray[T], error) {
	...
	case PTypeUint8:
		return encodeZigZagAs(arr, testRawChild[uint8])
	case PTypeUint16:
		return encodeZigZagAs(arr, testRawChild[uint16])
	case PTypeUint32:
		return encodeZigZagAs(arr, testRawChild[uint32])
	default:
		return encodeZigZagAs(arr, testRawChild[uint64])
	}
}
