...
func buildPrimitiveRunEndTest[V Integer | Float](arr array.ArrayCore[V], cmp cmpFn[V]) (EncodedArray[V], error) {
	switch n := arr.Length(); {
	case n <= 1<<8:
		return encodePrimitiveRunEndAs(arr, cmp, testRawChild[V], testRawChild[uint8], testBuildBudget)
	case n <= 1<<16:
		return encodePrimitiveRunEndAs(arr, cmp, testRawChild[V], testRawChild[uint16], testBuildBudget)
	case n <= 1<<32:
		return encodePrimitiveRunEndAs(arr, cmp, testRawChild[V], testRawChild[uint32], testBuildBudget)
	default:
		return encodePrimitiveRunEndAs(arr, cmp, testRawChild[V], testRawChild[uint64], testBuildBudget)
	}
}
...
func buildStringRunEndTest(arr array.ArrayCore[string], cmp cmpFn[string]) (EncodedArray[string], error) {
	switch n := arr.Length(); {
	case n <= 1<<8:
		return encodeStringRunEndAs(arr, cmp, testRawStringChild, testRawChild[uint8], testBuildBudget)
	case n <= 1<<16:
		return encodeStringRunEndAs(arr, cmp, testRawStringChild, testRawChild[uint16], testBuildBudget)
	case n <= 1<<32:
		return encodeStringRunEndAs(arr, cmp, testRawStringChild, testRawChild[uint32], testBuildBudget)
	default:
		return encodeStringRunEndAs(arr, cmp, testRawStringChild, testRawChild[uint64], testBuildBudget)
	}
}
