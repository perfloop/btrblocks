...
func encodeFoR[T Integer](arr array.ArrayCore[T], budget buildBudget) (EncodedArray[T], error) {
	if arr.Length() == 0 {
	...
	n := arr.Length()
	vals, err := makeBuildSlice[T](budget, n, n, "FoR source values")
	if err != nil {
		return nil, err
	}
	for i := range n {
	...
	child, err := buildBitPackedArrayWithWidth(array.NewVirtual(n, func(i uint64) T { return vals[i] - minValue }), rangeWidth, budget)
	if err != nil {
		return nil, err
	}
	return &forArray[T]{min: minValue, child: child}, nil
