...
func TestSparseRoundTrip(t *testing.T) {
	...
	encoded, err := encodeIntegerSparse(array.NewPrimitivesUnsafe(values), testSparseChildren[uint32]{}, testBuildBudget)
...
func TestSparsePreservesFloatNaNBits(t *testing.T) {
	...
	encoded, err := encodeFloatSparse(array.NewPrimitivesUnsafe(values), testSparseChildren[float64]{}, testBuildBudget)
	require.NoError(t, err)
	decoded, err := Decompress(encoded)
	require.NoError(t, err)
	for i, want := range values {
...
func TestSparseStringRoundTrip(t *testing.T) {
	...
	encoded, err := encodeStringSparse(mustStrings(t, values), testStringSparseChildren{}, testBuildBudget)
	require.NoError(t, err)
	decoded, err := Decompress(encoded)
	require.NoError(t, err)
	require.Equal(t, values, decoded)
...
func TestSparseConstantBuildStaysInsideCodec(t *testing.T) {
	values := array.NewPrimitivesUnsafe([]uint32{7, 7, 7, 7})
	children := testSparseChildren[uint32]{}
	encoded, err := encodeIntegerSparse(values, children, testBuildBudget)
	require.NoError(t, err)
	require.Equal(t, CodecTypeConst, encoded.CodecType())
	decoded, err := Decompress(encoded)
	require.NoError(t, err)
	require.Equal(t, []uint32{7, 7, 7, 7}, decoded)
}
