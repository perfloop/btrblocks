...
// compressChildCore materializes a structural child and sends it through the
// ordinary selector with a family-tagged error.
...
func buildPrimitiveRunEndTyped[V array.Integer | array.Float](arr array.ArrayCore[V], ctx planContext, compressValues childCompressor[V]) (codec.EncodedArray[V], error) {
	...
	case n <= 1<<16:
		return buildPrimitiveRunEndAs[V, uint16](arr, ctx, compressValues)
	case n <= 1<<32:
		return buildPrimitiveRunEndAs[V, uint32](arr, ctx, compressValues)
	default:
		return buildPrimitiveRunEndAs[V, uint64](arr, ctx, compressValues)
	}
}
