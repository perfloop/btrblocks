...
func encodePrimitiveRunEndAs[V Integer | Float, I UnsignedInteger](arr array.ArrayCore[V], cmp func(V, V) bool, buildRuns childBuilder[V], buildEnds childBuilder[I]) (EncodedArray[V], error) {
	return buildRunEndAs(arr, cmp, buildRuns, buildEnds, slicePrimitiveToRawArray[V])
}

// encodeStringRunEndAs extracts string runs and end positions and delegates
// both typed children to the caller.
func encodeStringRunEndAs[I UnsignedInteger](arr array.ArrayCore[string], cmp func(string, string) bool, buildRuns childBuilder[string], buildEnds childBuilder[I]) (EncodedArray[string], error) {
	return buildRunEndAs(arr, cmp, buildRuns, buildEnds, sliceStringToRawArray)
}

func buildRunEndAs[V Integer | Float | String, I UnsignedInteger](arr array.ArrayCore[V], cmp func(V, V) bool, buildRuns childBuilder[V], buildEnds childBuilder[I], slice sliceBuilder[V]) (EncodedArray[V], error) {
	...
	// Materialize to avoid per-element interface dispatch in the run scan.
	vals := make([]V, n)
	for i := range n {
		vals[i] = arr.ValueAt(i)
	}

	estRuns := max(int(n/8), 8)
